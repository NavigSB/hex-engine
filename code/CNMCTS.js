var StochasticArtificialPlayer = (function() {
	...
	function onMCTSMessage(msg) {
		...
		class MCTSChanceNode {
			constructor(numChildren, parent) {
				this.parent = parent;
				this.visits = 0;
				this.wins = 0;
				this.numUnexpandedMoves = 0;
				this.children = new Array(numChildren).fill(null); //temporary store move for debugging purposes
			}
		}
		...
		function selectMove() {
			...
			for (let i = 0; i < params.iterations; i++) {
				...
				//if selected node is terminal and we lost, make sure we never choose that move if possible
				if (game.gameOver()) {
		...
		function selectNode(root) {
			...
			while (root.numUnexpandedMoves == 0) {
				...
				let result = game.playMove(moves[maxIndex]);

				root = root.children[maxIndex];
				if(result) {
					let chosenIndex = result[0];
					if(!root.children[chosenIndex]) {
						root.children[chosenIndex] = new MCTSNode(game.moves(), root);
					}
					root = root.children[chosenIndex];
				}
		...
		function expandNode(node) {
			...
			let result = game.playMove(moves[childIndex]);
			moves = game.moves();
			let newNode;
			if(result) {
				//We need a chance node! We're gonna create and fully expand the chance node! 
				//Result is an array of 2 values: random event index and number of possible events
				let newChanceNode = new MCTSChanceNode(result[1], node);
				node.children[childIndex] = newChanceNode;
				//New chance node has no real children, so each time we go into a node we've
				//  never used, we are going to have to reinitialize it, even in selectNode
				newNode = new MCTSNode(moves, newChanceNode);
				newChanceNode.children[result[0]] = newNode;
			}else{
				newNode = new MCTSNode(moves, node);
